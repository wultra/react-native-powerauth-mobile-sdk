...
export class PowerAuth_ConfigureTests extends TestWithActivation {
    ...
    async runMethodsThatMustFail(sdk: PowerAuth) {
        ...
        await expect(async () => await sdk.hasProtocolUpgradeAvailable()).toThrow({errorCode: PowerAuthErrorCode.INSTANCE_NOT_CONFIGURED})
        await expect(async () => await sdk.hasPendingProtocolUpgrade()).toThrow({errorCode: PowerAuthErrorCode.INSTANCE_NOT_CONFIGURED})
        await expect(async () => await sdk.startProtocolUpgrade('')).toThrow({errorCode: PowerAuthErrorCode.INSTANCE_NOT_CONFIGURED})
        ...
        await expect(async () => await sdk.beginPasswordChange('')).toThrow({errorCode: PowerAuthErrorCode.INSTANCE_NOT_CONFIGURED})
